        ...
        for i, result in reversed(list(enumerate(results))):
            if int(result["battle_number"]) not in battles:
                if result["game_mode"]["key"] == "private" and custom_key_exists("ignore_private", True):
                    ...
                    mapname = translate_stages.get(translate_stages.get(int(result["stage"]["id"]), ""), fullname)
                    print("New battle result detected at {}! ({}, {})".format(
                        datetime.datetime.fromtimestamp(int(result["start_time"])).strftime('%I:%M:%S %p').lstrip("0"),
                        mapname, worl))
                battles.append(int(result["battle_number"]))
...
def export_to_excel(ally_scoreboard, battle_number, x_array, enemy_scoreboard, mystats, payload):
    ...
    name_sorted_ally_scoreboard[:] = [name_sorted_ally_scoreboard[i] for i in orderList]
    ...
    for i in range(len(name_sorted_enemy_scoreboard)):
        ...
        top_headers[player_name_header_position] = excelInfo.OPPONENT[lang_code] + str(i + 1)
        for ii in range(len(player_headers)):
            battle_headers = np.append(battle_headers, player_headers[ii])
    ...
    if worksheet_name in workbook.sheetnames:
        ...
        worksheet.row_dimensions[1].border = openpyxl.styles.Border(
            bottom=openpyxl.styles.Side(style="thin", color="00000000"))
        worksheet.row_dimensions[2].border = openpyxl.styles.Border(
            bottom=openpyxl.styles.Side(style="thin", color="00000000"))
...
def name_is_player(list_of_names, full_player_name):
    for name in list_of_names:
        if name in full_player_name:
            return True
    return False
