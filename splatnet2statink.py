    ...
    for i, result in reversed(list(enumerate(results))):
        ...
        if r_flag:
            if bn not in statink_battles: # one of the splatnet battles isn't on stat.ink (unuploaded)
                ...
                post_battle(0, [result], s_flag, t_flag, x_array, -1, True if i == 0 else False, debug, False)
...
def set_scoreboard(payload, battle_number, mystats, s_flag, x_array, battle_payload=None):
...
def export_to_excel(ally_scoreboard, battle_number, x_array, enemy_scoreboard, mystats, payload):
    ...
    if payload["lobby"] == "standard" and payload["mode"] == "regular":  # turf war
        ...
        if "estimate_x_power" in payload:
...
def post_battle(i, results, s_flag, t_flag, x_array, m_flag, sendgears, debug, ismonitor=False):
